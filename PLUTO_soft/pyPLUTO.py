...
hasYT=False
try:
    from pyevtk.hl import gridToVTK # This can be downloaded there: https://bitbucket.org/pauloh/pyevtk
    hasVTK=True
except ImportError:
    hasVTK=False
...
class pload(object):
    ...
    **Usage**:
    ...
    """
    def __init__(self, ns, w_dir=None, machine='', datatype=None, level=0, x1range=None, x2range=None, x3range=None,dec=1,silent=False,noload=False,debug_time=False):
        """Loads the data.
        ...
        **Outputs**:
        ...
        Data_dictionary = self.ReadDataFile(self.NStepStr,silent=silent,noload=noload)
        ...
        if (self.version == 3):
            ...
            if hasattr(self,'b1'):
                ...
                self.bx2 = self.b2
                self.bx3 = self.b3
        # to cope with version 4.3 and 4.4
        if hasattr(self,'Bx1'):
            self.bx1=self.Bx1
            self.bx2=self.Bx2
            self.bx3=self.Bx3
        ...
        **Output**:
        ...
        while True:
            ...
            try:
                ...
                if l.split()[0] == b'SCALARS':
                    ks.append((l.split()[1]).decode('utf-8'))
                elif l.split()[0] == b'LOOKUP_TABLE':
                    if noload:
                        fp.seek(nb,1)
    ...
    def DataScanHDF5(self, fp, myvars, ilev,noload=False):
    ...
    def ReadSingleFile(self, datafilename, myvars, n1, n2, n3, endian,
                       dtype, ddict,silent=False,noload=False):
        ...
        **Output**:
        ...
        if self.datatype == 'vtk':
            ...
            h5d = self.DataScanHDF5(fp,myvars,self.level,noload=noload)
            ddict.update(h5d)
            if hasYT:
            ...
            if (not silent) and (not noload):
                tmp=fp.read()
                if (len(tmp) > 0):
                    print ('Oh BOY, something went wrong because',len(tmp),'were not read')
    ...
    def ReadDataFile(self, num, silent=False, noload=False):
        ...
        **Outputs**:
        ...
        if self.fltype == "single_file":
            ...
            self.ReadSingleFile(datafilename, self.vars, self.n1, self.n2,
                                self.n3, endian, dtype, ddict,silent=silent,noload=noload)
...
class Image(object):
    ...
    def oplotbox(self, AMRLevel, lrange=[0,0], cval=['b','r','g','m','Orange','c','w','k'],\
